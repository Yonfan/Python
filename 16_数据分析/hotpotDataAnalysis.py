...
data = pd.read_csv(r'./dataDir/hotpotdata.csv')
...
sns.stripplot(x="star", y="environment", data=data, color='r', alpha=".25",
              order=['五星商户', '准五星商户', '四星商户', '准四星商户', '三星商户'])
plt.show()
...
tj = tuijian[(tuijian['comment'] > comment_low) & (tuijian['comment'] < comment_high) & (
        tuijian['avg_price'] > avg_price_low) & (tuijian['avg_price'] < avg_price_hign)].reset_index()
